"""Evaluación de v2_pinn: predice todo el grid por sample y reporta RMSE/R² en mT.

A diferencia de v1, el modelo es per-punto. Para evaluar comparablemente al
baseline iteramos sobre samples del split y para cada una hacemos un forward
sobre **todos los J puntos** de la grilla.
...
def evaluate(lit_model, h5_path, sample_indices,
    ...
    El modelo entrena en espacio normalizado (B per-componente, coords per-eje).
    Acá:
      1. Sensores normalizados + coords normalizadas → modelo.
      2. Salida normalizada → desnormalizada con `b_norm·b_std + b_mean`.
      3. Comparada contra `b_true` raw (mT) → RMSE/R² en unidades físicas.
    """
