"""HDF5 → RAM + scalers per-componente + PointDataset para v2_pinn.
...
grilla — Cada step de entrenamiento ve el mismo campo, lo
cual es necesario para que `\nabla \cdot B` y `∇×B` tengan sentido sobre el batch.

`PointDataset` levanta el split entero a RAM como
tensores torch float32 al inicializar y pre-aplica las normalizaciones —
`__getitem__` queda como indexing puro sin tocar el disco.

**Normalización**

  - sensores : `StandardScaler` per-feature (I·3 medias, I·3 escalas).
  - puntos   : `(xyz - pts_mean) / pts_std`, **vector (3,)** independiente
               por eje. Refleja que `Nz/Nx` no son simétricos.
  - B target : `(B - b_mean) / b_std`, **vector (3,)** independiente por
               componente. Refleja que `By` (eje principal del Halbach
               horizontal) tiene magnitud y std mucho mayor que `Bx, Bz`.
...
def compute_train_stats(h5_path, train_indices, *, chunk=64):
    ...
    - `b_mean`    : np.array shape (3,) — media de Bx, By, Bz sobre todos los
                    puntos de todos los samples de train.
    - `b_std`     : np.array shape (3,) — std de Bx, By, Bz idem.
    - `pts_mean`  : np.array shape (3,) — media de x, y, z sobre todos los
                    puntos del grid (idéntico para todos los samples).
    - `pts_std`   : np.array shape (3,) — std de x, y, z idem.

    Streaming sobre HDF5.
    """
    if chunk < 2:
    ...
    # Welford incremental sobre las 3 componentes de B (mean, M2 = sum(x - mean)**2).
    b_count = 0
